...
/*
  SELECT order_items.order_id, order_items.quantity, menu_items.name, menu_items.price, orders.status
  FROM order_items
...
*/
  ...
  let sqlQuery = 'SELECT order_items.order_id, order_items.quantity, menu_items.name, menu_items.price, orders.status ';
  ...
  //sqlQuery += 'AND orders.status = $2;';

  return pool
    .query(sqlQuery, values)
    .then((items) => {
      //console.log("ORDER ITEMS inside getOrderItems function: ", items.rows )
      return items.rows; 
    })
    .catch(err => err.messages);
